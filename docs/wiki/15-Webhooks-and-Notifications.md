...
| 状況 | ステータス | 備考 |
|------|------------|------|
| 不明な ID / 非アクティブ / シークレット未設定 / 署名不正 | **401** `{"success":false,"error":"Unauthorized"}` | Webhook の有無を推測されにくくするため意図的に揃える。**シークレット未設定の検知に 503 を期待しないこと**（管理画面または `GET /api/webhooks/incoming` で確認） |
| JSON 不正 | 400 | |
| ペイロード過大 | 413 | |
| レート超過 | 429 | グローバル／ID 別バケット |

受信成功時の処理:
1. 署名検証済みボディを重複排除テーブルで判定
2. 初回のみイベントバスに `incoming_webhook.{source_type}` を発火
3. オートメーション・スコアリング・通知ルールが連動して処理
