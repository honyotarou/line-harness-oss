# TDD Step 0〜3 — 観点 → Red → Green → Refactor
...
**チェック**: [line-harness-harness](../line-harness-harness/SKILL.md) の「モノレポの地図」と矛盾しないか。
...
1. Step 0 で決めたファイルに **最小の `it` / `describe`** を書く。
...
**禁止**: このステップで「とりあえず実装を入れて緑にする」こと（それは Step 2）。

---

## Step 2 — Green（最小実装）

**目的**: Step 1 のテストを **満たす最小差分**で通す。
...
- **新しい振る舞いは追加しない**（追加するなら Step 0 に戻る）。
...
必ず **Step 4**（層別ゲート）に進み、`pnpm harness` で緑を確認する。
