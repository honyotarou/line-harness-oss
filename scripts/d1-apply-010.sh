...
if [ "$TARGET" = "remote" ]; then
  ...
  pnpm exec wrangler "${WR_EXTRA[@]}" d1 execute line-crm --remote --yes --file="../../packages/db/migrations/010_users_unique_contact.sql"
else
  echo "== d1-apply-010: LOCAL $FILE =="
  pnpm exec wrangler "${WR_EXTRA[@]}" d1 execute line-crm --local --yes --file="../../packages/db/migrations/010_users_unique_contact.sql"
fi
