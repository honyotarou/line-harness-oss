...
export function createFriendsResource(
  ...
  count: (params?: { accountId?: string }) => Promise<number>;
...
export class FriendsResource {
  private readonly api: ReturnType<typeof createFriendsResource>;
  constructor(http: HttpClient, defaultAccountId?: string) {
    this.api = createFriendsResource(http, defaultAccountId);
  }
  ...
  async addTag(friendId: string, tagId: string): Promise<void> {
    return this.api.addTag(friendId, tagId);
  }

  async removeTag(friendId: string, tagId: string): Promise<void> {
    return this.api.removeTag(friendId, tagId);
  }
  ...
  async sendMessage(
    ...
    return this.api.sendMessage(friendId, content, messageType);
  }

  async setMetadata(friendId: string, fields: Record<string, unknown>): Promise<Friend> {
    return this.api.setMetadata(friendId, fields);
  }

  async setRichMenu(friendId: string, richMenuId: string): Promise<void> {
    return this.api.setRichMenu(friendId, richMenuId);
  }

  async removeRichMenu(friendId: string): Promise<void> {
    return this.api.removeRichMenu(friendId);
  }
}
