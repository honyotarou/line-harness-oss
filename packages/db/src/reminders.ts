...
export async function createReminderStep(
  ...
  await db
    ...
    .run();
  return (await db
    .prepare(`SELECT * FROM reminder_steps WHERE id = ?`)
    .bind(id)
    .first<ReminderStepRow>())!;
}
...
export async function enrollFriendInReminder(
  ...
  await db
    ...
    .run();
  return (await db
    .prepare(`SELECT * FROM friend_reminders WHERE id = ?`)
    .bind(id)
...
export async function completeReminderIfDone(
  ...
  const deliveredSteps = await db
    .prepare(
      `SELECT COUNT(*) as count FROM friend_reminder_deliveries WHERE friend_reminder_id = ?`,
    )
