...
export async function getAutomations(db: D1Database): Promise<AutomationRow[]> {
  const result = await db
    .prepare(`SELECT * FROM automations ORDER BY priority DESC, created_at DESC`)
    .all<AutomationRow>();
...
export async function getActiveAutomationsByEvent(
  db: D1Database,
  eventType: string,
): Promise<AutomationRow[]> {
  const result = await db
    .prepare(
      `SELECT * FROM automations WHERE event_type = ? AND is_active = 1 ORDER BY priority DESC`,
    )
    .bind(eventType)
    .all<AutomationRow>();
  return result.results;
