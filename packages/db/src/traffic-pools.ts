...
export type TrafficPoolWithAccount = Readonly<
  TrafficPool & {
    account_name: string;
    liff_id: string | null;
...
export async function createTrafficPool(
  ...
  // Keep pool_accounts in sync for fresh installs (migration 019 only backfills existing rows).
  ...
  await db
    ...
    .bind(paId, id, input.activeAccountId, now)
    .run();
...
export type PoolAccount = Readonly<{
  id: string;
  pool_id: string;
...
export type PoolAccountWithDetails = Readonly<
  PoolAccount & {
    account_name: string;
    liff_id: string | null;
    login_channel_id: string | null;
...
export async function getRandomPoolAccount(
  db: D1Database,
  poolId: string,
): Promise<PoolAccountWithDetails | null> {
  return db
    .prepare(
      `${POOL_ACCOUNT_JOIN} WHERE pa.pool_id = ? AND pa.is_active = 1 ORDER BY RANDOM() LIMIT 1`,
    )
    .bind(poolId)
...
export async function addPoolAccount(
  ...
  const result = await db
    .prepare(
      'INSERT INTO pool_accounts (id, pool_id, line_account_id, is_active, created_at) VALUES (?, ?, ?, 1, ?) RETURNING *',
    )
