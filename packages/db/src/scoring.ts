...
export async function getScoringRuleById(
  db: D1Database,
  id: string,
): Promise<ScoringRuleRow | null> {
...
/** 友だちのスコア履歴を取得 */
export async function getFriendScoreHistory(
  db: D1Database,
  friendId: string,
): Promise<FriendScoreRow[]> {
  const result = await db
...
export async function getActiveRulesByEvent(
  db: D1Database,
  eventType: string,
): Promise<ScoringRuleRow[]> {
  const result = await db
